        ...
        if check:
            ...
            if file_type.lower() == "c":
                f.write("MCU=atmega88\n"
                        ...
                        ",--section-start=.mmcu=0x910000 -I ../simavr/simavr/sim "
                ...
                f.write("MCU=atmega88\n"
                        "CFLAGS=-Wall -gdwarf-2 -Os -std=gnu99 -DF_CPU=8000000 -fno-inline-small-functions "
                        "-ffunction-sections -fdata-sections -Wl,--relax,--gc-sections -Wl,--undefined=_mmcu"
                        ",--section-start=.mmcu=0x910000 -I ../simavr/simavr/sim/avr\n"
                        "CC=avr-g++\n"
                        "OBJCOPY=avr-objcopy\n\n"
                        f"TARGET={file_name}\n\n"
                        "$(TARGET).hex: $(TARGET).elf\n\t"
...
def upload(a):
    ...
    p = subprocess.Popen('../simavr/simavr/run_avr --mcu atmega88 --freq 8000000 {}.elf'.format(name), shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
