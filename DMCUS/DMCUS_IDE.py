        ...
        if check:
            ...
            if file_type.lower() == "c":
                f.write("MCU=atmega328p\n"
                        ...
                        "$(OBJCOPY) -j .text -j .data -j .eeprom -O ihex $(TARGET).axf $(TARGET).hex\n\n"
                        "$(TARGET).axf: $(TARGET).c\n\t"
                        "$(CC) $(CFLAGS) -mmcu=$(MCU) $(TARGET).c -o $(TARGET).axf\n\n"
                        "clean:\n\t"
                        "rm -f *.axf *.hex")
                f.close()
                ...
                f.write("MCU=atmega328p\n"
                        ...
                        "$(TARGET).hex: $(TARGET).axf\n\t"
                        "$(OBJCOPY) -j .text -j .data -j .eeprom -O ihex $(TARGET).axf $(TARGET).hex\n\n"
                        "$(TARGET).axf: $(TARGET).cpp\n\t"
                        "$(CC) $(CFLAGS) -mmcu=$(MCU) $(TARGET).cpp -o $(TARGET).axf\n\n"
                        "clean:\n\t"
                        "rm -f *.axf *.hex")
                f.close()
                p = subprocess.Popen(['make'], shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
